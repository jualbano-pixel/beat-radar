...
type TopStory = PriorityOutputStory & {
  why_it_matters: string;
};

type SecondarySignal = PriorityOutputStory & {
  note: string;
};

type PriorityScoredStory = {
...
function buildCoverageMap(stories: NormalizedStory[]): Map<string, NormalizedStory[]> {
  const coverageMap = new Map<string, NormalizedStory[]>();
...
function getRecencyBoost(publishedAt: string, latestDate: number): number {
...
function buildWhyItMatters(reasons: string[]): string {
...
function getBreadthSignals(text: string): string[] {
...
function scoreStory(
  ...
  if (
    countKeywordHits(text, IMPACT_KEYWORDS.partnership) > 0 &&
    countKeywordHits(text, IMPACT_KEYWORDS.major_ai_company) > 0
  ) {
    score += 3;
  ...
  if (
    countKeywordHits(text, STRATEGIC_KEYWORDS.finance_strategy) > 0 &&
    countKeywordHits(text, IMPACT_KEYWORDS.major_ai_company) > 0
  ) {
    score += 2;
  ...
  if (
    countKeywordHits(text, STRATEGIC_KEYWORDS.research_ecosystem) > 0 &&
  ...
  if (
    countKeywordHits(text, ROUTINE_PRODUCT_TERMS) > 0 &&
    countKeywordHits(text, BROADER_IMPACT_TERMS) === 0
  ) {
    score -= 3;
  ...
  if (
    reasons.includes("foundational_research") &&
...
function toOutputStory(scored: PriorityScoredStory): PriorityOutputStory {
  return {
...
export function selectTopStories(stories: NormalizedStory[]): {
  ...
  const latestDate = Math.max(
    ...stories.map((story) => new Date(story.publishedAt).getTime())
  );
  const scoredStories = stories
    .map((story) => {
      const topicCoverage =
        [...coverageMap.values()].find((group) => group.includes(story))?.length ?? 1;
  ...
  const topStories: PriorityScoredStory[] = [];
  const secondarySignals: PriorityScoredStory[] = [];
