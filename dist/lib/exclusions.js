...
exports.matchesHardExclusion = matchesHardExclusion;
exports.applyHardExclusions = applyHardExclusions;
...
function hasConsumerProductContext(value) {
...
function matchesHardExclusion(story) {
    ...
    const dealIntentMatch = /\bdeal(s)?\b/i.test(title) &&
    ...
    const affiliateListicleMatch = (/\bbest\b/i.test(title) || /\bguide\b/i.test(title) || /\bwe tested\b/i.test(title)) &&
        hasConsumerProductContext(titleAndUrl);
    const summaryCommerceMatch = hasConsumerProductContext(titleAndUrl) &&
        matchesAnyPattern(summary, SUMMARY_COMMERCE_PATTERNS);
    const eventPromoMatch = matchesAnyPattern(title, EVENT_PROMO_TITLE_PATTERNS) &&
...
function applyHardExclusions(stories) {
    const kept = [];
    const dropped = [];
    for (const story of stories) {
        const result = matchesHardExclusion(story);
        if (result.excluded && result.reason) {
            dropped.push(buildDrop(story, result.reason, result.reason === "hard_excluded_commerce"
